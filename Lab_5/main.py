...
# ─────────────────────────────────────────────
# Пункт 2 — точне значення інтегралу (scipy)
# ─────────────────────────────────────────────
def exact_integral():
...
# ─────────────────────────────────────────────
# Пункт 3 — складова формула Сімпсона
# ─────────────────────────────────────────────
def simpson(func, a, b, N):
    """
    Складова формула Сімпсона.
    N — кількість вузлів (має бути парним).
    """
    if N % 2 != 0:
        N += 1  # гарантуємо парність
    h = (b - a) / N
...
# ─────────────────────────────────────────────
# Пункт 4 — залежність точності від N
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# Пункт 5 — похибка при N0
# ─────────────────────────────────────────────
def compute_N0(N_opt):
    """N0 ≈ N_opt/10, кратне 8"""
    raw = N_opt // 10
...
def runge_romberg(I0, N0):
    """
    Уточнення по Рунге-Ромбергу для формули Сімпсона (p=4).
    I_R = I(N0) + [I(N0) - I(N0/2)] / (2^4 - 1)
    """
...
def aitken(I0, N0):
    """
    Три кроки: N0, N0/2, N0/4  →  I1, I2, I3
    Порядок:  p = log2((I1-I2)/(I2-I3))
    Уточнення: I_A = I3 + (I3-I2)/(2^p - 1)
    """
...
def adaptive_simpson(func, a, b, eps, depth=0, max_depth=50):
    """
    Рекурсивний адаптивний алгоритм на основі формули Сімпсона.
...
def study_adaptive(I0):
    """Досліджуємо залежність точності і кількості обчислень від eps."""
